...
export const AdminProfileModal: React.FC<AdminProfileModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4">
      <div 
        className="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-150"
        role="dialog"
        aria-modal="true"
      >
        {/* Header */}
            ...
            <div>
              <div className="flex items-center space-x-2">
                ...
                <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${
                  ...
                  {isSuperAdmin ? 'Admin Tổng (Toàn Quyền)' : 'Cấp Dưới (Quản Lý Tài Xế)'}
                </span>
              </div>
          ...
          <div className="mt-3 p-2.5 rounded-xl bg-slate-800/80 border border-slate-700 text-xs text-slate-300 leading-relaxed">
            {isSuperAdmin ? (
              ...
              <span className="text-indigo-300 font-medium">
                👤 Cấp dưới: Giới hạn chỉ thao tác mục Tài xế (Thêm, sửa, cấp mũ/áo, thu/hoàn cọc).
              </span>
            )}
          </div>
        </div>
