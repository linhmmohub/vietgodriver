...
import { DriverAttendanceEvent } from '../types';
...
export const AttendancePerformancePanel: React.FC<Props> = ({ events, selectedDate }) => {
  const now = Date.now();
  ...
  const performance = useMemo(() => {
    ...
    return [...grouped.entries()].map(([driverId, driverEvents]) => {
      ...
      return { driverId, driverName: first.driverName, driverCode: first.driverCode, totalMs, completedSessions, isActive: activeStart !== null, lastEvent: ordered[ordered.length - 1] };
      ...
      <div className="mt-4 grid gap-2 lg:grid-cols-2">
        {performance.length === 0 ? <p className="rounded-xl border border-dashed border-slate-700 p-4 text-center text-xs text-slate-400">Chưa có lần vào/ra ca nào trong ngày được chọn.</p> : performance.map(item => (
          <div key={item.driverId} className="rounded-xl border border-slate-700 bg-slate-800/80 p-3">
            <div className="flex items-start justify-between gap-2"><div><p className="text-sm font-bold text-white">{item.driverName}</p><p className="text-[10px] font-mono text-slate-400">{item.driverCode}</p></div><span className={`rounded-full px-2 py-1 text-[10px] font-bold ${item.isActive ? 'bg-emerald-500/15 text-emerald-300' : 'bg-slate-700 text-slate-300'}`}>{item.isActive ? 'Đang trong ca' : `${item.completedSessions} ca xong`}</span></div>
            <p className="mt-2 text-xs font-bold text-cyan-200">{formatDuration(item.totalMs)} làm việc</p>
            <p className="mt-1 text-[10px] text-slate-400">Lần cuối: {eventLabel(item.lastEvent.eventType)} · {new Date(item.lastEvent.occurredAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</p>
          </div>
        ))}
      </div>
